...
/**
 * Contains automobile_Data_Importer class
 *
 * @since	1.2
 * @package	WordPress
 */
...
/**
 * Class automobile_Data_Importer class
 *
 * @since	1.2
 */
class automobile_Data_Importer {
    ...
    function __construct() {
        ...
        $this->automobile_importer_class_path = WP_PLUGIN_DIR . '/cs-automobile-framework/includes/cs-importer/wordpress-importer.php';
    }
    ...
    function import() {
        ...
        if ($this->is_widgets) {
            ...
            ob_end_clean();
            if ($this->action_return) {
                $this->make_output(true, __('Widgets successfully got imported.', 'cs-frame'));
            } else {
                $this->make_output(false, __('Sorry widgets class missing.', 'cs-frame'));
            }
        }
        ...
        if ($this->is_theme_options) {
            ...
            ob_end_clean();
            if ($this->action_return) {
                $this->make_output(true, __('Theme options successfully got imported.', 'cs-frame'));
            } else {
                $this->make_output(false, __('Sorry theme options file not readable.', 'cs-frame'));
            }
        }
        ...
        if ($this->is_users) {
            ...
            ob_end_clean();
            if ($this->action_return) {
                $this->make_output(true, __('Users successfully got imported.', 'cs-frame'));
            } else {
                $this->make_output(false, __('Sorry users file not readable.', 'cs-frame'));
            }
        }
        ...
        if ($this->is_menus) {
            ...
            ob_end_clean();
            if ($this->action_return) {
                $this->make_output(true, __('Menus successfully got imported.', 'cs-frame'));
            } else {
                $this->make_output(false, __('Sorry menus was not imported.', 'cs-frame'));
            }
        }
        ...
        if ($this->is_plugins) {
            ...
            ob_end_clean();
            if ($this->action_return) {
                $this->make_output(true, __('Plugin Options successfully got imported.', 'cs-frame'));
            } else {
                $this->make_output(false, __('Sorry plugin options was not imported.', 'cs-frame'));
            }
        }
        ...
        if ($this->is_sliders) {
            ...
            ob_end_clean();
            if ($this->action_return) {
                $this->make_output(true, __('Sliders successfully got imported.', 'cs-frame'));
            } else {
                $this->make_output(false, __('Sorry sliders was not imported.', 'cs-frame'));
            }
        }
    }
     ...
     * invoke automobile_import_wp_data action
     */
    function import_wp_data() {
        ...
        do_action('automobile_import_wp_data', $this);
    ...
    /**
     * Import Widgets
     * invoke automobile_import_widgets
     */
    function import_widgets() {
        do_action('automobile_import_widgets', $this);
    }

    /**
     * Import Theme Options
     * invoke automobile_import_theme_options
     */
    function import_theme_options() {
        do_action('automobile_import_theme_options', $this);
    }

    /**
     * Import Users
     * invoke automobile_import_users
     */
    function import_users() {
        do_action('automobile_import_users', $this);
    }

    /**
     * Import Menus and Locations
     * invoke automobile_import_menus_and_locations
     */
    function import_menus_and_locations() {
        do_action('automobile_import_menus_and_locations', $this);
    }

    /**
     * Import Plugin Options
     * invoke automobile_import_plugin_options hook
     */
    function import_plugin_options() {
        do_action('automobile_import_plugin_options', $this);
    }

    /**
     * Import Sliders
     * invoke automobile_import_rev_sliders
     */
    function import_sliders() {
        do_action('automobile_import_rev_sliders', $this);
    }

    /**
     * Set up Pages, set home page for the WP site
     * invoke automobile_import_setup_pages
     */
    function set_up_pages() {
        do_action('automobile_import_setup_pages', $this);
    }
    
    /**
     * Delete Default Content
     * invoke automobile_delete_default_content
     */
    function delete_default_content() {
        do_action('automobile_delete_default_content');
    }
