 ...
 * </code>
 * Example : Note : define your Prefix (like tabs || tab_item ) in pattren. (ref. automobile_get_pattern())
 ...
 *  [/tabs]';
 * 	$Output = array();
 *  $PREFIX = 'prefix'; //user prefix as automobile_message OR automobile_tabs | tab_item
 * 	$parseInstance 	= new ShortcodeParse();
 * 	$output = $b->automobile_shortcodes( $output, $text );
 * 	echo '<pre>';
...
if (!class_exists('ShortcodeParse')) {
    ...
    class ShortcodeParse {
        ...
        function automobile_get_pattern($content, $PREFIX) {
        ...
        function automobile_parse_atts($content) {
        ...
        function automobile_shortcodes(&$output, $content = '', $child = true, $PREFIX = '') {
            $patts = $this->automobile_get_pattern($content, $PREFIX);
            $t = array_filter($this->automobile_get_pattern($content, $PREFIX));
            if (!empty($t)) {
                ...
                foreach ($parents as $k => $parent) {
                    ...
                    $t = array_filter($this->automobile_get_pattern($contents[$k], $PREFIX));
                    $t_s = $this->automobile_shortcodes($outputNew, $contents[$k], true, $PREFIX);
                    $output[$name] = array('name' => $parents[$k]);
                    $output[$name]['atts'] = $this->automobile_parse_atts($atts[$k]);
