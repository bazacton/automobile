...
if (!function_exists('automobile_create_transactions_fields')) {
    ...
    function automobile_create_transactions_fields($key, $param) {
        global $post, $automobile_html_fields, $automobile_form_fields, $automobile_plugin_options, $automobile_var_plugin_static_text;
        $strings = new automobile_plugin_all_strings;
            $strings->automobile_var_plugin_option_strings();
        
        $automobile_gateway_options = get_option('automobile_plugin_options');
        ...
        switch ($param['type']) {
            ...
            case 'summary' :
                ...
                if (isset($transaction_pay_method) && $transaction_pay_method != '') {
                    ...
                    $logo = $automobile_plugin_options[strtolower($transaction_pay_method) . '_logo'];
                    if (isset($logo) && $logo != '') {
            ...
            case 'cv_resumes' :
                // prepare
                $automobile_inventory_id = get_post_meta($post->ID, "automobile_resume_ids", true);
                $automobile_inventory_ids = $automobile_inventory_id;
                $automobile_inventory_id = explode(',', $automobile_inventory_id);
                $automobile_post_data = '';
                $automobile_saved_ads = array();
                if (is_array($automobile_inventory_id) && $automobile_inventory_id[0] == '')
                    unset($automobile_inventory_id[0]);
                if (isset($automobile_inventory_id) && is_array($automobile_inventory_id) && sizeof($automobile_inventory_id) && !empty($automobile_inventory_id)) {
                    foreach ($automobile_inventory_id as $id) {
                        $automobile_user_data = get_userdata($id);
                        if (!empty($automobile_user_data)) {
                            $automobile_permalink = get_dashboard_url($id);
                            $automobile_title = $automobile_user_data->display_name;
                            $automobile_post = '<ul>';
                            $automobile_post .= '<li>' . automobile_var_plugin_text_srt('automobile_var_resume_id') . ' : #' . $id . '</li>';
                            $automobile_post .= '<li>' . automobile_var_plugin_text_srt('automobile_var_resume_title') . ' : <a target="_blank" href="' . esc_url($automobile_permalink) . '">' . $automobile_title . '</a></li>';
                            $automobile_post .= '</ul>';
                            $automobile_post_data .= '<span>' . $automobile_post . '</span>';
                        }
                        if ($automobile_permalink <> '') {
                            $automobile_saved_ads[] = $id;
                        }
                    }
                } else {
                    $automobile_post_data .= automobile_var_plugin_text_srt('automobile_var_no_resumes_used_yet');
                }
                if (is_array($automobile_saved_ads) && sizeof($automobile_saved_ads) > 0) {
                    $automobile_inventory_ids = implode(',', $automobile_saved_ads);
                }
